...
#define TOUCHCAL_CA  0x82 //0x2A
#define TOUCHCAL_CB  0x99B3 //0x3CD4 //0xE67E
#define TOUCHCAL_CC  0xD1 //0x31
#define TOUCHCAL_CD  0xCAD1 //0x28F //0xE944
#define CALIBRATIONINSET   10  
...
//////////////////////// GLOBAL VARIABLES ////////////////////////////
    ...
    // HardwareProfile.h
    #define TOUCHSCREEN_RESISTIVE_PRESS_THRESHOLD  4090	// between 0-0x03ff the lesser this value 
...
volatile SHORT      adcX_Old = -1;
volatile SHORT      adcY_Old = -1;
...
// and the multiplication will yield incorrect values.
    ...
    #define TOUCHSCREEN_RESISTIVE_CALIBRATION_SCALE_FACTOR 10
#endif
...
LECTURAS_de_X:
     ...
     if(ADS7843_PIN_IRQ){
               ...
               temp_x = ADS7843WriteAndReadData(ADS7843_READ_X);
	                 if(temp_x != tempX)
	                 {                            
	                    tempX = temp_x;
	                    goto LECTURAS_de_X;
...
LECTURAS_de_Y:
     ...
     if(ADS7843_PIN_IRQ){
              ...
              temp_y = (4096 - ADS7843WriteAndReadData(ADS7843_READ_Y));
             ...
             if(((0x50 <= tempX) && (tempX <= 0xFA0)) && ((0x50 <= tempY) &&(tempY <= 0xFA0)))
                {  
                 if((abs(tempX - adcX_Old)>10) && (abs(tempY - adcY_Old)>10)){
...
/*
    // Initialize ADC
    AD1CON1 = 0x00E0;       // Turn on, auto-convert
    AD1CON2 = 0;            // AVdd, AVss, int every conversion, MUXA only
    AD1CON3 = 0xFFFF;       // 31 Tad auto-sample, Tad = 256*Tcy
 ...
 */  
    ...
    //AD1CON1 = 0x0FFF;       // Turn on, auto-convert
    AD1CON2 = 0;            // AVdd, AVss, int every conversion, MUXA only
    AD1CON3 = 0xFFFF;       // 31 Tad auto-sample, Tad = 256*Tcy
...
void TouchLoadCalibration(void)
{
    ...
    if (pCalDataRead = NULL)//**if (pCalDataRead != NULL)
	{
         //**
    	_trA = 0x22F5; //0xE98D; //= pCalDataRead(ADDRESS_RESISTIVE_TOUCH_COEFA);
    	_trB = 0x07C0; //0xB93; //= pCalDataRead(ADDRESS_RESISTIVE_TOUCH_COEFB);
    	_trC = 0x348B;  //0x0; //= pCalDataRead(ADDRESS_RESISTIVE_TOUCH_COEFC);
    	_trD = 0xFC16;  //0x3250; //= pCalDataRead(ADDRESS_RESISTIVE_TOUCH_COEFD);
         //**
...
void TouchCalHWGetPoints(void)
{
    ...
    XCHAR   calStr1[] = {'e','n',' ','e','l',' ','m','e','d','i','o',' ','d','e','l',0};
    XCHAR   calStr2[] = {'c','i','r','c','l','o',0};
    XCHAR  	calTouchPress[] = {'P','r','e','s','i','o','n','e',0};
    XCHAR   calTouchHold[] = {'M','a','n','t','e','n','e','r',0};
    XCHAR   calTouchRelease[] = {'S','o','l','t','a','r',0};
    SHORT   counter;
