...
WORD SldDraw(void *pObj)
{
    ...
    switch(state)
    {
        case SLD_STATE_IDLE:
            ...
            {
                return (1); //Para que no pinte el panel Y SEA VERDADERAMENTE HIDEN

                SetColor(pSld->hdr.pGolScheme->CommonBkColor);      // set to common BK Color
                //SetColor(RGBConvert(0, 0, 0));   //Color del fondo cuando esta activado
        ...
        case SLD_STATE_THUMBPATH1:
            ...
            if(!GetState(pSld, SLD_VERTICAL))
            {
                ...
                //SetColor(RGBConvert(152, 152, 152));
                //if(!FillBevel(minPos, midPoint-3, maxPos, midPoint+3, 0))  //Linea inicial, cuando se dibuja por primera vez
        ...
        case SLD_STATE_THUMBPATH2:
            ...
            if(!GetState(pSld, SLD_VERTICAL))
            {
                if(!Line(minPos, midPoint + 1, maxPos, midPoint + 1)) //linea que se dibuja por abajo de la linea principal
              //  SetColor(RGBConvert(1, 152, 248));
              //  if(!FillBevel(minPos, midPoint-3, maxPos, midPoint+3, 0))
                    return (0);
            }
            else
            {
        ...
        case SLD_STATE_CLEARTHUMB:              // this removes the current thumb
            ...
            if(!GetState(pSld, SLD_VERTICAL))
            {
                return (1); //Para que no pinte el panel Y SEA VERDADERAMENTE HIDEN CUANDO EL SLIDER ES VERTICAL

                if(!Bar(pSld->prevPos - thWidth, midPoint - thHeight, pSld->prevPos + thWidth, midPoint + thHeight))
                //if(!FillCircle(pSld->prevPos, midPoint,thWidth ))
                    return (0);
            }
            else
            {
                 return (1); //Para que no pinte el panel Y SEA VERDADERAMENTE HIDEN CUANDO EL SLIDER ES HORIZONTAL
        ...
        case SLD_STATE_REDRAWPATH1:         // redraws the lines that it covered
            ...
            if(!GetState(pSld, SLD_VERTICAL))
            {
                ...
                //SetColor(RGBConvert(96, 152, 248));
                //if(!FillBevel(left, midPoint-2, right, midPoint+2, 0))
                    return (0);
            }
            else
            {
        ...
        case SLD_STATE_REDRAWPATH2:
            ...
            if(!GetState(pSld, SLD_VERTICAL))
            {
                ...
                //SetColor(RGBConvert(96, 152, 248));
                //if(!FillBevel(left, midPoint-3, right, midPoint+3, 0)) //Linea de izquierda a derecha
