        ...
        switch(state)
        {
            ...
            case ST_STATE_CLEANAREA:
                ...
                //if(!Bar(pSt->hdr.left + 1, pSt->hdr.top + 1, pSt->hdr.right - 1, pSt->hdr.bottom - 1))
                 //   return (0);
                //********************Modificacion para que dibuje el frame en gradiente**********************//
         // while(!BarGradient(pSt->hdr.left + 1, pSt->hdr.top + 1, pSt->hdr.right - 1, pSt->hdr.bottom - 1,(RGBConvert(255, 255, 255)),(RGBConvert(0, 0, 255)), 50, 3));
                  //return (0);
                //********************************************************************************************//
            ...
            case ST_STATE_SETALIGN:
                if(charCtr == 0)
                {
                    ...
                    {
                        MoveTo((pSt->hdr.left + pSt->hdr.right - textWidth) >> 1, pSt->hdr.top + (lineCtr * pSt->textHeight)-10);   //** EL -10 SE LO PUSE PARA SUBIR UN POCO EL TEXTO
                    }
    
                    // Display text with right alignment
                    else if(GetState(pSt, (ST_RIGHT_ALIGN)))           //MODIFICO ESTA PARA QUE CUANDO PONGA, ST_RIGHT_ALIGN, ME UBIQUE EL TEXTO CENTRADO EN LA PARTE INFERIOR
                    {
                        //**MoveTo((pSt->hdr.right - textWidth - ST_INDENT), pSt->hdr.top + (lineCtr * pSt->textHeight));
                        MoveTo((pSt->hdr.left + pSt->hdr.right - textWidth) >> 1, 143 + (lineCtr * pSt->textHeight));      //153 ES LA POSICION EN Y DEL TEXTO
                    }
            ...
            case ST_STATE_DRAWTEXT:
                ...
                else
                {
                    ...
                    CreatePrimitivesForventana_1();   //Para que me actualize los text de la ventana uno
                    return (1);
                }
        } // end of switch()
    } // end of while(1)    
